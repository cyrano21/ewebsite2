...
interface EcomTopDealsProps {
  products: any[];
  className?: string;
}

const EcomTopDeals: React.FC<EcomTopDealsProps> = ({ products = [], className = '' }) => {
  return (
    <Card className={`h-100 ${className}`}>
      <Card.Header className="d-flex justify-content-between align-items-center bg-light">
        <h3 className="mb-0">Meilleures offres</h3>
        <Link href="/customer/products/top-deals" className="btn btn-sm btn-outline-primary">
          Voir tout
        </Link>
      </Card.Header>
      <Card.Body>
        {products.length === 0 ? (
          <div className="text-center py-4">
            <p className="text-muted mb-0">Aucun produit disponible pour le moment</p>
          </div>
        ) : (
          <Row className="g-3">
            {products.slice(0, 6).map((product, index) => (
              <Col key={product.id || index} xs={12} sm={6} md={4}>
