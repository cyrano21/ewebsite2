...
import styles from './ProductWidgets.module.css'; // Utiliser le même CSS Module

const BestSellingProducts = ({ products, loading, error }) => {
  if (loading) {
    return (
  ...
  if (!products || products.length === 0) {
    return (
      <Card className="h-100 border-light shadow-sm">
        <Card.Header className="bg-light py-2"><h6 className="mb-0 fw-normal">Meilleures Ventes</h6></Card.Header>
  ...
  return (
    <Card className="h-100 border-light shadow-sm">
      <Card.Header className="bg-light py-2">
        <h6 className="mb-0 fw-normal">Meilleures Ventes</h6>
      </Card.Header>
      <ListGroup variant="flush" className={styles.productList}>
        {products.map((product) => (
          <ListGroup.Item key={product.id} className={`${styles.productItem} d-flex align-items-center`}>
            ...
            <Badge pill bg="success" text="white" className={styles.productMetric}>
...
BestSellingProducts.defaultProps = {
  products: [],
  loading: false,
  error: null
};
