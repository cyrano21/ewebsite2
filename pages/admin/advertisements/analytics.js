  ...
  const fetchAnalytics = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        console.log("Données analytiques chargées avec succès:", response.data.data);
      } else {
        console.error("Erreur de l'API:", response.data);
        setError(`Erreur lors de la récupération des données d'analytique: ${response.data.message || 'Erreur inconnue'}`);
      }
    } catch (err) {
      console.error('Erreur lors de la récupération des données d\'analytique:', err);
      const errorMessage = err.response?.data?.message || err.message || 'Erreur inconnue';
      setError(`Une erreur s'est produite lors de la récupération des données d'analytique: ${errorMessage}`);
    } finally {
      setLoading(false);
    }
  };
