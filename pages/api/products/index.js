...
export default async function handler(req, res) {
  ...
  if (method === 'GET') {
    ...
    if (category && category !== 'all' && category !== 'undefined') {
      ...
      if (mongoose.Types.ObjectId.isValid(category)) {
        ...
        if (catDoc && catDoc._id) {
          ...
          // Si pas trouvé, retourner un tableau vide directement
          console.log('[API products] Catégorie non trouvée');
    ...
    let rawProducts = [];
    try {
      rawProducts = await Product.find(filter).populate('category', 'name').limit(limitNum).lean();
      console.log('[API products] Nombre de produits trouvés:', rawProducts.length);
    } catch (error) {
      console.error('[API products] Erreur lors de la requête:', error);
  ...
  res.status(405).json({ error: 'Méthode non autorisée' });
    ...
    console.error('[API products] Erreur globale:', error);
