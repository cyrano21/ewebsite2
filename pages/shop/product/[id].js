      ...
      <Head>
        ...
        <meta
          ...
          content={
            ...
            `Détails pour ${product.name || "Product"}`
          }
        />
      </Head>
      <Container className="product-page-container pt-4 pb-5">
        ...
        <Row className="mt-4 description-tabs-section">
          ...
          <Col lg={4}>
            ...
            <Tags
              tags={product.tags || []}
              selectedTag={null} // Vous pouvez gérer l'état sélectionné si besoin
              onTagClick={(tag) => {
                // Exemple : naviguer vers une page de tag ou filtrer les produits
                // router.push(`/shop?tag=${encodeURIComponent(tag)}`);
                // Pour l’instant, affichage simple
                alert(`Tag sélectionné : ${tag}`);
              }}
