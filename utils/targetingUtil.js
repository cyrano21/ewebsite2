...
export const useRelevantAds = (position, type, limit = 1, rotationGroup = 'default', enableRotation = true) => {
  ...
  useEffect(() => {
    ...
    const fetchAds = async () => {
      ...
      if (fetchController.current) {
        try {
          ...
          console.log('Erreur lors de l\'annulation de la requête précédente:', abortError);
        }
      }
      ...
      try {
        ...
        if (error.name === 'AbortError') {
          console.log('Requête abandonnée:', error.message || 'suite à un changement de dépendances');
          return;
        }
    ...
    return () => {
      ...
      if (fetchController.current) {
        try {
          fetchController.current.abort('Composant démonté');
        } catch (error) {
          // Ignorer les erreurs d'abandon pendant le nettoyage
          console.log('Nettoyage du contrôleur abandonné');
        }
